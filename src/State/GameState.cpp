...
entt::handle GameState::CreateEntity(Nz::EnttWorld& world, const Nz::Vector2f& position, const std::string& name) {
	...
	if (name == "Pad") {
		...
		sprite->SetColor(Nz::Color(255, 255, 255));
	}
	else if (name == "Ball"){
		sprite->SetSize(BALL_SIZE);
		sprite->SetColor(Nz::Color(255, 0, 0));
	}
...
void GameState::ResetBall() {
	...
	//std::cout << std::format("x velocity : {}, y velocity : {}", std::cos(Nz::DegreeToRadian(angle)), -std::sin(Nz::DegreeToRadian(angle))) << std::endl;
...
bool isInCollision(entt::handle& obj1, entt::handle& obj2) {
	...
	//std::cout << std::format("ball({},{}) obj2({},{})", PosObj1.x, PosObj1.y, PosObj2.x, PosObj2.y) << std::endl;
...
void GameState::checkPhysics() {
	//std::array<entt::handle, 6> collidableObjects{ m_leftPad, m_rightPad, m_upperBord, m_lowerBord, m_leftBord, m_rightBord };
